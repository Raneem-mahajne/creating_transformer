...
**Conclusions and implications.** We have shown that a transformer constrained to $n_{\mathrm{embed}} = 2$ can solve the plus-last-even task by implementing a purely geometric algorithm. By inspecting the embeddings, Q/K/V projections, attention patterns, and output regions in two dimensions, we can explain how the model detects the `+` token, retrieves the most recent even number, and converts that information into the correct prediction via a vector shift that moves the representation into the appropriate decision zone of the LM head. This offers a concrete example of mechanistic interpretability: rather than only observing what the model predicts, we can trace exactly how the computation is organized inside the network and verify the algorithm without the need for dimensionality reduction.

**Limitations.** The primary limitation of this approach is the representational capacity of the 2D plane. While $\mathbb{R}^2$ is sufficient for the 12-token plus-last-even task, more complex conditional rules would likely result in overlapping representations and degraded performance. Furthermore, this study is restricted to a single-layer, single-head architecture. In deeper models, the interaction between successive residual updates and FFN transformations may introduce topological complexities that are harder to interpret as a single-step geometric nudge toward a decision boundary.

**Future directions.** We plan to use this 2D framework to investigate how the model’s statistical learning compares to the transitional probabilities tracked by the brain during sequence processing. By training the transformer on more complex patterns, we can see if its geometric solutions reflect the same statistical shortcuts used in biological neural networks. 
