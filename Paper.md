...
2. **Detect the operator.** The query projection $W_Q$ maps the `+` embedding to a query vector that is geometrically distinct from number queries (Figure 9, top).

3. **Retrieve the last even number.** The dot product between the `+` query and all keys (produced by the $W_K$ projection) yields high attention weight for even-digit keys, with recency encoded in the positional component of the key layout. The focused-query analysis (Figure 9, bottom) and full attention matrix (Figure 10) make this retrieval pattern explicit; the per-query gradient figure (Figure 11) shows it for the demo sequence.
