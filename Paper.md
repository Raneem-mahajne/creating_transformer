...
**Conclusions and implications.** For the plus-last-even task, a tiny transformer does not just achieve the right output; it learns an internal geometry that can be read as an algorithm. By inspecting embeddings, Q/K/V projections, attention patterns, and output regions in two dimensions, we can explain how the model detects the `+` token, retrieves the most recent even number, and converts that information into the correct prediction. This is useful because it offers a concrete example of mechanistic interpretability: rather than only observing what the model predicts, we can trace how the computation is organized inside the network.

**Limitations.** The setting is deliberately simple: the task is synthetic, the vocabulary is small, and the model has only one block and one head. That simplicity is what makes the geometry interpretable, but it also limits how far the conclusions can be generalized to larger models or natural-language tasks. In addition, the algorithmic account given here is an interpretation supported by consistent geometric evidence, not a formal proof, and we have not systematically studied how stable these structures are across tasks, architectures, or random seeds.

**Future directions.** The most important next step is to test whether this style of analysis extends to harder algorithmic tasks and to somewhat larger models, where computation may be distributed less cleanly. It would also be useful to compare runs across seeds and model sizes to identify which geometric features are robust, and to automate parts of the analysis so that similar studies scale more easily. Finally, targeted interventions on embeddings or attention pathways could help establish whether the proposed geometric mechanism is not just descriptive but genuinely causal.
