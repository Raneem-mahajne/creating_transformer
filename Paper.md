...
Figure 7 provides a compact summary of the landscape shown in Figure 6. Panel (a) shows the *entropy* of the output distribution: the upper half-plane has high entropy (many tokens are plausible), while the lower zones have near-zero entropy (a single even number dominates). Panel (b) shows the *argmax prediction map*: at every point in the plane, the color indicates which token the output network would predict most strongly.
...
Panels (c) and (d) of Figure 7 overlay the model's internal representations on the argmax map (panel b) to reveal where they sit relative to the output network's decision regions. In panel (c), the 96 combined embeddings $\mathbf{e}_i$ (12 tokens $\times$ 8 positions) are plotted. The digits embeddings generaly fall in the high-entropy indifference region in the upper half-plane (visible in panel a), where no specific retrieval is triggered. In contrast, all `+` embeddings land in the lower half-plane — inside the strong prediction region — which provides a geometric headstart for the algorithm: the model has already categorized `+` as requiring an even-number output based on token identity and position alone. However, because the residual stream at this stage lacks historical context, it can identify the *type* of output needed but cannot resolve the specific value. This defines the attention layer's objective: it must look back to find the most recent even number and produce a value vector that, when added to the residual stream, nudges the representation into the correct even-number target zone.

Panel (d) shows the value-transformed vectors $W_V \mathbf{e}_j$ for all 96 combined token+positions, overlaid on the same argmax map. The positioning of these value vectors relative to the decision boundaries. Note that the ordering of the value vectors on the plane in figure 7d is the same order as the even-digit prediction zones in figure 7b. This illustrates that the purpose of the value vectors is to nudge the embeddings towards a particular zone in order to produce the correct output. However, *which* value specifically will be chosen depends on the attention matrix which requires a specific sequence.

![Output Landscape Summary](plus_last_even/plots/a4/07_output_landscape_summary.png)
***Figure 7.** Output landscape summary (summary of Figure 6). (a) Entropy of the output distribution (nats); high in the indifference region (top half), near zero in even-number target zones (bottom half). (b) Argmax prediction map: color and annotation indicate which token has the highest predicted probability at each point. (c) Argmax map with combined embeddings $\mathbf{e}_i$ overlaid. (d) Argmax map with value-transformed vectors $W_V \mathbf{e}_i$ overlaid.*
